...
PhysicsObject::PhysicsObject(Transform* parentTransform, const CollisionVolume* parentVolume, PhysicsType type, float Elasticity)	{
...
void PhysicsObject::AddForceAtPosition(const Vector3& addedForce, const Vector3& position) {
	...
	force  += addedForce;
	torque += Vector3::Cross(localPos, addedForce); // Why don't need to suit the Conversation of Energy?
}
